...
доступным в момент ``run_init_utc + delay``. Значение берется с запасом над
наблюдаемым временем публикации. Наблюдение пока одно: метаданные Open-Meteo
за 23.09.2026, история публикаций не проверялась, у IFS и GFS запас всего 30 мин.
...
SOURCES: dict[str, Source] = {
    ...
    for source in (
        Source(
            ...
            cache_dir="ifs",
            delay_basis=(
                "Решение команды (#2, GOAL.md). Для февраля 2026 консервативно: шаги 0-90 всех четырех прогонов ECMWF рассылает к +6:12, "
                "по метаданным Open-Meteo прогон 00z 23.09.2026 стал доступен через 7.0 ч. "
                "Для 2024-2025 это допущение «доступность как сейчас»: до 01.10.2025 IFS 9 км открыто в реальном времени не публиковался, "
                "открытые данные 0.25° шли на 2 ч позже (шаг 72 около +8:12, см. ifs025). "
                "С исторической задержкой обучение шло бы на 12z, а симуляция на 18z."
            ),
        ),
        Source(
            name="ifs025",
